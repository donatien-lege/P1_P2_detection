...
def eval(pred_path, curve_path, annot_path, out_path):
    ...
    #curvature, prediction and annotation files
    ...
    # Delete pulses without a P1/P2 ratio
    ...
    # Horizontal MAE
    factor = snakemake.config["size"]/100
    df_error["hz_P1"] = (preds[:, 0] - peaks[:, 0])/factor
    df_error["hz_P2"] = (preds[:, 1] - peaks[:, 1])/factor
